...
The dataset is constructed from the open-access Crystallography Open Database (COD) using real experimental diffraction data. Each training pair is generated by matching the final expert-resolved structure with the coarse electron density map obtained from initial phasing, ensuring direct alignment with the model’s training objective. To provide reliable supervision, we retain only structures for which the final refined solution can be confidently matched to its corresponding initial phasing result. The resulting dataset comprises over 50,000 structures, spanning organic, organometallic, and inorganic crystals, and covers 83 elements across 86 space groups.
