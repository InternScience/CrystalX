...
def load_shelxt(res_file_path, begin_flag = 'PLAN', end_flag = 'HKLF', is_atom_name = False, is_check_sfac=False):
    qpeak = []
    coord = []
    shelxt_pred = []
    qvalue_list = []
        ...
        for line in res_file:
            ...
            if start_reading:
                if 'HKLF' in line:
                    ...
                    if p >= len(atom_list):
                        # If predicted atom list is shorter than PLAN..HKLF atom rows,
                        # drop remaining tail rows to keep output consistent with prediction length.
                        tail_trimmed += 1
                        continue
                    atom_pos = line.split()
            ...
            if 'PLAN' in line:
                ...
                start_reading = True
    if tail_trimmed > 0:
        print(f"[WARN] update_shelxt: trimmed tail atom rows={tail_trimmed} (pred_n={len(atom_list)})")
...
def _pick_afix_code(atom_type, h, degree):
    """
    Choose AFIX code for one heavy atom based on predicted H count and local degree.
    """
...
def _afix_uiso_scale(code):
    # Typical riding-model Uiso scale in SHELXL:
    # methyl-like groups use 1.5 (negative convention), others 1.2.
    if int(code) in (137,):
        return -1.5
    return -1.2


def gen_afix_ins(ins_file_path, mol_graph, hydro_num):
    """
    Build per-atom AFIX blocks directly from model predictions.
    Returns:
      afix_by_atom: dict[str, list[str]]
      stats: prediction/AFIX hydrogen-count consistency stats
    """
    # Prefer SHELXL-refined format (FVAR->HKLF) so hydro mapping follows *_AI.res.
    atom_frac = []
    atom_name = []
    try:
        atom_frac_fvar, atom_name_fvar = load_shelxt_final(
            ins_file_path, begin_flag='FVAR', is_atom_name=True, is_hydro=False
        )
        if len(atom_name_fvar) > 0:
            atom_frac = atom_frac_fvar
            atom_name = atom_name_fvar
    except Exception:
        pass

    if len(atom_name) == 0:
        atom_frac_anis, atom_name_anis, _, _ = load_shelxt(
            ins_file_path, begin_flag='ANIS', is_atom_name=True
        )
        filtered_frac = []
        filtered_name = []
        for p, name in zip(atom_frac_anis, atom_name_anis):
            atom_sym = extract_non_numeric_prefix(name).upper()
            if atom_sym == 'H':
                continue
            filtered_frac.append(p)
            filtered_name.append(name)
        atom_frac = filtered_frac
        atom_name = filtered_name

    if hasattr(hydro_num, "detach"):
        hydro_num = hydro_num.detach().cpu().tolist()
    elif isinstance(hydro_num, np.ndarray):
        hydro_num = hydro_num.tolist()
    else:
        hydro_num = list(hydro_num)

    if len(atom_name) != len(hydro_num):
        raise ValueError(
            f"Hydrogen prediction length mismatch for {ins_file_path}: "
            f"template_heavy_atoms={len(atom_name)} vs predicted={len(hydro_num)}"
        )

    frac_by_atom = {name: atom_frac[i] for i, name in enumerate(atom_name)}

    h_sfac_idx = None
    with open(ins_file_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            s = line.strip()
            if s.upper().startswith('SFAC '):
                sfac_list = [x.capitalize() for x in s.split()[1:]]
                if 'H' in sfac_list:
                    h_sfac_idx = sfac_list.index('H') + 1
                break
    if h_sfac_idx is None:
        # Keep going with a conservative fallback used by existing files.
        h_sfac_idx = 6
    ...
    def _next_h_labels(parent_atom, n_h):
        nonlocal h_serial
        # Prefer SHELXL-like naming: H<parentIndex>, H<parentIndex>A/B...
        m = re.search(r'(\d+)', parent_atom)
        parent_idx = m.group(1) if m else str(h_serial)
        out = []
        if n_h <= 1:
            cand = f'H{parent_idx}'
            if cand in used_h_labels:
                cand = f'H{h_serial}'
                h_serial += 1
            out.append(cand)
        else:
            ...
            for i in range(n_h):
                suffix = letters[i] if i < len(letters) else str(i + 1)
                cand = f'H{parent_idx}{suffix}'
                if cand in used_h_labels:
                    cand = f'H{h_serial}'
                    h_serial += 1
                out.append(cand)
    ...
    for atom, h in zip(atom_name, hydro_num):
        ...
        degree = len(connect_atom)
        atom_type = extract_non_numeric_prefix(atom).upper()

        left = h
        chunk_codes = []
        while left > 0:
            chunk = min(left, 3)
            code = _pick_afix_code(atom_type, chunk, degree)
            if code is None:
                break
            c_h = AFIX_HCOUNT.get(code, 0)
            # Ensure strict count consistency with model prediction chunk.
            if c_h != chunk:
                fallback = {1: 43, 2: 23, 3: 137}.get(chunk)
                if fallback is None:
                    break
                code = fallback
                c_h = AFIX_HCOUNT.get(code, 0)
        ...
        if left > 0 or not chunk_codes:
            skipped_atoms.append({"atom": atom, "pred_h": h, "atom_type": atom_type, "degree": degree})
            per_atom.append({"atom": atom, "pred_h": h, "afix_codes": [], "implied_h": 0})
            continue

        block_lines = []
        h_labels = []
        xh, yh, zh = frac_by_atom.get(atom, [0.0, 0.0, 0.0])
        for code in chunk_codes:
            n_h = AFIX_HCOUNT.get(code, 0)
            uiso = _afix_uiso_scale(code)
            block_lines.append(f'AFIX {int(code):>3d}\n')
            new_h_labels = _next_h_labels(atom, n_h)
            for h_label in new_h_labels:
                h_labels.append(h_label)
                block_lines.append(
                    f'{h_label:<4} {int(h_sfac_idx):>2d}    '
                    f'{float(xh):.6f}    {float(yh):.6f}    {float(zh):.6f}    '
                    f'11.00000   {float(uiso):.5f}\n'
                )
                implied_total += 1
            block_lines.append(f'AFIX {0:>3d}\n')

        afix_by_atom[atom] = block_lines

        per_atom.append(
            {
                "atom": atom,
                "pred_h": h,
                "afix_codes": chunk_codes,
                "implied_h": int(sum(AFIX_HCOUNT.get(c, 0) for c in chunk_codes)),
                "h_labels": h_labels,
            }
        )

    stats = {
        "pred_total_h": int(pred_total),
        "afix_implied_total_h": int(implied_total),
        "pred_vs_afix_match": bool(int(pred_total) == int(implied_total)),
        "skipped_atom_count": int(len(skipped_atoms)),
        "skipped_atoms": skipped_atoms,
        "per_atom": per_atom,
    }
    return afix_by_atom, stats


def gen_hfix_ins(ins_file_path, mol_graph, hydro_num):
    atom_name = []
    # .res written by SHELXL usually stores atom table after FVAR (not ANIS).
    try:
    ...
    # Backward-compatible fallback for files where atom table starts after ANIS.
    if len(atom_name) == 0:
        _, atom_name = load_shelxt(ins_file_path, begin_flag='ANIS', is_atom_name=True)
...
def update_shelxt_hydro_afix(res_file_path, new_res_file_path, afix_by_atom):
    """
    Write hydro instructions as AFIX blocks placed right after each heavy atom block.
    Existing AFIX/HFIX/H-atom records in the atom section are removed before insertion.
    """

    def _is_atom_record(parts):
        if len(parts) < 5:
            return False
        if not any(ch.isalpha() for ch in parts[0]):
            return False
        try:
            float(parts[1])
            float(parts[2])
    ...
    inserted_atoms = []
    requested_atoms = [k for k, v in afix_by_atom.items() if v]

    with open(res_file_path, 'r') as res_file, open(new_res_file_path, 'w') as new_res_file:
        in_atom_section = False
        skip_existing_conf_htab = False
        skip_h_continuation = False
        pending_atom_label = None
        pending_wait_cont = False

        def _flush_pending_afix(force=False):
            nonlocal pending_atom_label, pending_wait_cont
            if pending_atom_label is None:
                return
            if (not force) and pending_wait_cont:
                return
            block = afix_by_atom.get(pending_atom_label, [])
            for afix_line in block:
                new_res_file.write(afix_line)
            if block:
                inserted_atoms.append(pending_atom_label)
            pending_atom_label = None
            pending_wait_cont = False
        ...
        for line in res_file:
            ...
            if skip_existing_conf_htab:
                if head in ('CONF', 'HTAB'):
                    continue
                if head:
                    skip_existing_conf_htab = False

            if (not in_atom_section) and head == 'BOND':
                new_res_file.write('BOND $H\n')
                new_res_file.write('CONF\n')
                new_res_file.write('HTAB\n')
                skip_existing_conf_htab = True
                continue
            ...
            if in_atom_section:
                if head == 'HKLF':
                    _flush_pending_afix(force=True)
                    in_atom_section = False
                    new_res_file.write(line)
                    continue

                # Drop previous hydro-control records; we will rewrite from model outputs.
                if head in ('AFIX', 'HFIX'):
                    continue

                # If a skipped H atom had anisotropic continuation, drop that one line as well.
                if skip_h_continuation:
                    if not _is_atom_record(parts):
                        skip_h_continuation = False
                        continue
                    skip_h_continuation = False

                if _is_atom_record(parts):
                    atom_label = parts[0]
                    atom_type = extract_non_numeric_prefix(atom_label).upper()

                    # Remove explicit hydrogen atom lines from previous runs.
                    is_h = (atom_type == 'H')
                    if (not is_h) and (h_sfac_idx is not None):
                        try:
                            sfac_id = int(float(parts[1]))
                            if sfac_id == h_sfac_idx and atom_type.startswith('H'):
                                is_h = True
                        except Exception:
                            pass
                    if is_h:
                        if line.rstrip().endswith('='):
                            skip_h_continuation = True
                        continue

                    # Previous pending atom had no continuation line; flush before next heavy atom.
                    _flush_pending_afix(force=True)

                    new_res_file.write(line)
                    if atom_label in afix_by_atom:
                        pending_atom_label = atom_label
                        pending_wait_cont = line.rstrip().endswith('=')
                        _flush_pending_afix(force=False)
                    continue

                # Non-atom line in atom section (typically anisotropic continuation).
                new_res_file.write(line)
                if pending_atom_label is not None:
                    pending_wait_cont = False
                    _flush_pending_afix(force=True)
                continue
    ...
    inserted_set = set(inserted_atoms)
    missing_atoms = [a for a in requested_atoms if a not in inserted_set]
    return {
        "requested_atom_count": int(len(requested_atoms)),
        "inserted_atom_count": int(len(inserted_set)),
        "missing_atom_count": int(len(missing_atoms)),
        "missing_atoms": missing_atoms,
        "inserted_atoms": sorted(inserted_set),
    }
